import {commonMessages} from './messages/common'
import {buildPageMessages} from './messages/pages'

const menu = {
  Dashboard: '仪表盘',
  UserManagement: '用户管理',
  CoinMerchantModule: '币商管理',
  AnchorGuildManagement: '主播与工会管理',
  UserList: '用户列表',
  UserDetail: '用户详情',
  AnchorListManagement: '主播列表',
  AnchorDetail: '主播详情',
  LiveRoomRecycleBinManagement: '主播间回收站',
  BotAnchorManagement: '机器人主播',
  CoinMerchantManagement: '币商列表',
  RechargeOrderList: '充值订单',
  SimulatorDeviceWhitelistManagement: '模拟器白名单',
  GoldCurrencyLogList: '金币流水',
  DiamondCurrencyLogList: '钻石流水',
  CoinMerchantTransferLogList: '币商转账日志',
  OperationManagement: '运营管理',
  OperationContentGroup: '运营内容',
  OperationRechargeGroup: '充值会员',
  OperationGuildBasicGroup: '工会与主播',
  OperationGuildAccessGroup: '工会账号权限',
  OperationGuildDataGroup: '工会数据',
  OperationSettlementGroup: '结算配置',
  OperationAppGroup: '应用配置',
  BannerManagement: '首页Banner',
  ActivityMessageManagement: '活动消息',
  GuildManagement: '工会列表',
  GuildTransferManagement: '普通工会代付',
  CoinMerchantGuildTransferManagement: '币商代付',
  CoinMerchantPayoutDetailList: '币商代付详情',
  GuildCMSUserManagement: '工会CMS用户',
  GuildVisibilityManagement: '工会可见性',
  PlatformAnchorVisibilityManagement: '平台主播可见性',
  GuildAnchorDailyLiveManagement: '名下主播每日流水',
  GuildMembers: '工会成员',
  GuildDetail: '工会详情',
  PlatformAnchorList: '平台主播',
  GuildRecycleBinManagement: '工会垃圾库',
  GuildProfileManagement: '工会数据查询',
  RechargeCfgManagement: '充值配置',
  CoinMerchantRechargeCfgManagement: '币商充值档位',
  VipCfgManagement: 'VIP配置',
  AppPkgManagement: 'App包管理',
  RandomNicknameManagement: '随机昵称库',
  CustomerServiceCfgManagement: '客服联系配置',
  WalletExchangeCfgManagement: '结算基础配置',
  PaymentCountryCfgManagement: '普通用户代收支付配置',
  CoinMerchantPaymentCountryCfgManagement: '币商代收支付配置',
  LiveManagement: '直播管理',
  GiftManagement: '礼物管理',
  AgoraCfgManagement: '声网配置',
  PrivateRoomBillingManagement: '1v1通话计费',
  LiveCfgManagement: '直播配置',
  LiveRoomTagManagement: '直播间标签',
  LogManagement: '日志',
  LiveLogGroup: '社交日志',
  LiveRevenueLogList: '直播收益流水',
  LiveRecordList: '直播记录',
  LiveDailyEffectiveLiveList: '每日流水',
  LiveWeeklyUnsettledLiveList: '本周流水',
  CallLogGroup: '通话日志',
  VideoCallLogList: '视频通话日志',
  UserLogGroup: '用户日志',
  SettlementLogGroup: '主播与工会结算管理',
  GameLogGroup: '游戏日志',
  ShortVideoGroup: '短视频',
  ShortVideoManagement: '短视频管理',
  ShortVideoCategoryManagement: '短视频分类',
  ShortVideoPriceTierManagement: '短视频价格挡位',
  ShortVideoCfgManagement: '短视频配置',
  ShortVideoWatchManagement: '短视频观看记录',
  GameManagement: '游戏管理',
  GamePlatformCfgManagement: '平台接入配置',
  GameVendorGameListManagement: '游戏库',
  GameShelfListManagement: '上架游戏列表',
  GameVendorConfig: '第三方配置',
  GameBetLogListManagement: '游戏消费记录',
  GameWinLogListManagement: '游戏奖励记录',
  ActivityManagement: '活动管理',
  FirstRechargeActivityManagement: '首充活动配置',
  InviteRechargeRewardManagement: '邀请充值返还',
  ConfigManagement: '系统配置',
  ConfigBasicGroup: '基础配置',
  ConfigSecurityGroup: '安全合规',
  ConfigPlatformGroup: '平台与资源',
  ConfigDeployGroup: '前端管理',
  FrontendConfigGroup: '前端配置',
  FrontendDeployGroup: '前端部署',
  ConfigOpsGroup: '运维监控',
  AppTokenConfig: 'App Token',
  AccountCfgManagement: '账号配置',
  AppVersionCfgManagement: 'App版本查询',
  SimulatorCpuKeywordManagement: '模拟器CPU关键词',
  DeviceRegisterRiskCfgManagement: '设备码注册风控',
  AnchorSalaryCfgManagement: '主播结算薪资',
  AnchorSalarySocialShareCfgManagement: '有底薪社交流水分佣配置',
  AnchorNoSalaryShareCfgManagement: '无底薪社交流水分佣配置',
  AnchorSalaryGameShareCfgManagement: '有底薪游戏流水档位分佣配置',
  AnchorNoSalaryGameShareCfgManagement: '无底薪游戏流水档位分佣配置',
  LiveRevenueShareCfgManagement: '流水分佣配置',
  AnchorIncomeSettlementLogList: '主播结算流水',
  PlatformAnchorPayoutList: '平台主播代付',
  GuildIncomeSettlementLogList: '工会结算流水',
  GuildPayoutDetailList: '工会代付详情',
  PlatformAnchorPayoutDetailList: '平台主播代付详情',
  PreloadCfgManagement: '服务器运行配置',
  ServerRuntimeCfgManagement: '服务器运行配置',
  StaticCacheCfgManagement: '静态网页缓存',
  TextModerationCfgManagement: '敏感词过滤',
  PrivacyPolicyCfgManagement: '隐私政策',
  GooglePlayCfgManagement: 'Google Play',
  FirebaseCfgManagement: 'Firebase登录',
  HaiPayCfgManagement: 'HaiPay支付',
  CfEmailCfgManagement: 'Amazon SES邮件',
  DbBackupCfgManagement: '数据库备份',
  UploadResourceCfgManagement: '资源域名',
  CMSDomainMappingManagement: 'CMS域名映射',
  H5LiveDeployManagement: 'H5直播部署',
  CoinMerchantDeployManagement: '币商H5部署',
  CountryFlagDeployManagement: '国旗资源部署',
  ThirdPayDeployManagement: '第三方支付部署',
  OfficialSiteDeployManagement: '官网部署',
  ThirdPayOfficialSiteDeployManagement: '第三方支付官网部署',
  DataSyncCfgManagement: '数据同步',
  ResourceMonitor: '资源监控',
  ServerLogExplorer: '服务器日志',
  RoleManagementGroup: '权限管理',
  RoleManagement: '角色权限管理',
  ModuleList: '权限配置',
  CMSUserManagement: 'CMS用户管理',
  BanUser: '封禁用户',
  Login: '登录',
} as const

export default {
  common: commonMessages['zh-CN'],
  login: {
    title: 'Sara Live CMS',
    subtitle: '直播运营管理系统',
    welcome: '欢迎回来',
    hint: '请登录您的管理账号',
    userName: '用户名',
    userNamePlaceholder: '请输入用户名',
    password: '密码',
    passwordPlaceholder: '请输入密码',
    submit: '登录',
    success: '登录成功',
    userNameRequired: '请输入用户名',
    passwordRequired: '请输入密码',
    passwordMin: '密码长度不能少于6位',
    featureLive: '直播运营',
    featureData: '数据洞察',
    featureSecure: '安全可靠',
  },
  tabs: {
    actions: '标签操作',
    closeOthers: '关闭其他',
    closeAll: '关闭全部',
    close: '关闭',
  },
  menu,
  pages: buildPageMessages('zh-CN'),
}
