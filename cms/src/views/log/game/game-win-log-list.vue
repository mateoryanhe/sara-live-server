...
import {CMS_EXPORT_TYPE_GAME_WIN_LOG} from '@/utils/cms-async-export'
import {buildGameWinLogCsvColumns} from '@/utils/game-log-csv'

const {t} = useI18n()
const {can} = usePagePermission('GameWinLogListManagement')
const {canViewUserDetail, openUserDetail} = useUserDetailNav('GameWinLogListManagement')
...
const handleExport = async () => {
  await runExport(
    CMS_EXPORT_TYPE_GAME_WIN_LOG,
    {
      headers: buildCsvHeaders(buildGameWinLogCsvColumns(t)),
      ...buildFilterParams(),
    },
    `game-win-log-${Date.now()}.csv`,
  )
}
