...
1. 编译Go程序为Linux可执行文件
2. 打包可执行文件和配置文件
3. 上传并解压后:有进程则 curl 热重启,无进程则 start.sh 冷启动
4. 上传新的部署包到远程服务器
5. 解压并在远程服务器上启动新程序
...
- 一键部署.bat: 用户友好的启动脚本
- restart.bat: 仅重启正式服现有程序，不编译、不打包、不上传文件
- 一键重启.bat: 双击执行正式服重启并保留执行结果窗口
- README.txt: 本说明文件
...
1. 双击"一键部署.bat"运行脚本
2. 脚本将自动完成整个部署过程
3. 仅需重启正式服时，双击"一键重启.bat"；有运行进程时执行热重启，无进程时使用服务器现有 start.sh 启动
