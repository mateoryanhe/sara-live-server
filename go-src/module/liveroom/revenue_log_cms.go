...
func revenueLogToCMSItem(v *entity.LiveRevenueLog, profileMap map[uint64]*userentity.UserInfo) *liverevenuedto.CMSLiveRevenueLogItem {
	if v == nil {
		return nil
	}
...
func GetRevenueLogCMSList(_ context.Context, req *liverevenuedto.CMSLiveRevenueLogListReq) (*httpserver.CMSQueryResp, error) {
	...
	for _, row := range rows {
		list = append(list, revenueLogToCMSItem(row, profileMap))
	}
