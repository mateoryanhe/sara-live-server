...
// CreateCoinMerchantChannelRechargeOrder 币商App用yhpay建单(需登录;档位来自币商充值配置缓存)
func CreateCoinMerchantChannelRechargeOrder(ctx context.Context, req *rechargeorderdto.AppCreateCoinMerchantChannelRechargeOrderReq) (*rechargeorderdto.AppCreateChannelRechargeOrderRes, error) {
	...
	packageName := strings.TrimSpace(httpserver.GetPackageNameFromContext(ctx))
	return createCoinMerchantChannelRechargeOrder(ctx, userId, packageName, req.CfgId, req.CurrencyCode)
}

func createCoinMerchantChannelRechargeOrder(ctx context.Context, userId uint64, packageName string, cfgId uint64, currencyCode string) (*rechargeorderdto.AppCreateChannelRechargeOrderRes, error) {
	packageName = strings.TrimSpace(packageName)
	if userId == 0 || cfgId == 0 {
		return nil, errercode.CreateCode(errercode.InvalidParam)
	}
	...
	playerName := strconv.FormatUint(userId, 10)
	if u := userinfodao.GetUserInfoByUserId(userId); u != nil && strings.TrimSpace(u.Nickname) != "" {
		playerName = strings.TrimSpace(u.Nickname)
	}
	playerIP := "127.0.0.1"
	if r := g.RequestFromCtx(ctx); r != nil {
		if ip := strings.TrimSpace(r.GetClientIp()); ip != "" {
	...
	payUrl, err := createYhPayManualDeposit(ctx, cfg, orderIdStr, playerName, playerIP, currencyCode, payAmount)
	if err != nil {
		xrlog.DetailLog.Errorf(ctx, "yhpay coinMerchant create pay url failed orderId=%s userId=%d currency=%s payAmount=%v err=%v",
			orderIdStr, userId, currencyCode, payAmount, err)
		return nil, errercode.CreateCode(errercode.YhPayCreateFailed)
	}
	xrlog.DetailLog.Infof(ctx, "yhpay coinMerchant create pay url ok orderId=%s userId=%d currency=%s priceUsd=%v payAmount=%v",
		orderIdStr, userId, currencyCode, order.Price, payAmount)
