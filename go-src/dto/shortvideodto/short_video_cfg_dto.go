...
type ShortVideoCfgItem struct {
	...
	AnchorDailyUploadLimit     uint32 `json:"anchorDailyUploadLimit"`
	NormalUserDailyUploadLimit uint32 `json:"normalUserDailyUploadLimit"`
...
type SaveShortVideoCfgReq struct {
	...
	FreeWatchSeconds           uint32 `json:"freeWatchSeconds" v:"min:0#免费观看时长不能小于0" dc:"免费观看时长(秒),0表示无免费时长"`
	EntryEnabled               uint8  `json:"entryEnabled" v:"in:0,1#入口开关取值无效" dc:"入口开关(0关闭,1开启)"`
	AnchorDailyUploadLimit     uint32 `json:"anchorDailyUploadLimit" v:"required|min:1#主播每日上传上限不能为空|主播每日上传上限必须大于0" dc:"主播每日最大上传数量"`
	NormalUserDailyUploadLimit uint32 `json:"normalUserDailyUploadLimit" v:"required|min:1#普通用户每日上传上限不能为空|普通用户每日上传上限必须大于0" dc:"普通用户(类型0)每日最大上传数量"`
}
