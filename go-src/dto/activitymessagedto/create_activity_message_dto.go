...
type CreateActivityMessageReq struct {
	g.Meta    `path:"/createActivityMessage" method:"post" summary:"创建活动消息" tags:"活动消息"`
	...
	IconId    string `json:"iconId" v:"max-length:255#图标资源名(印尼语)最长255字符" dc:"图标资源名(印尼语)"`
	...
	BgId      string `json:"bgId" v:"max-length:255#背景图资源名(印尼语)最长255字符" dc:"背景图资源名(印尼语)"`
	...
	TitleId   string `json:"titleId" v:"max-length:128#标题(印尼语)最长128字符" dc:"标题(印尼语)"`
	...
	UrlId     string `json:"urlId" v:"max-length:512#跳转链接(印尼语)最长512字符" dc:"跳转链接(印尼语)"`
}

type CreateActivityMessageRes struct {
