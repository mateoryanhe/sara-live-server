...
type CMSAddDiamondReq struct {
	...
	Amount float64 `json:"amount" v:"required|min:0.001#钻石数量不能为空|钻石数量必须大于0" dc:"增加数量(正数)"`
}
...
type CMSSubDiamondReq struct {
	...
	UserId uint64  `json:"userId" v:"required#用户ID不能为空" dc:"用户ID"`
	Amount float64 `json:"amount" v:"required|min:0.001#钻石数量不能为空|钻石数量必须大于0" dc:"扣减数量(正数)"`
}
