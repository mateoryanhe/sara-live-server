...
var userCacheMgr *cache.RowCache[*entity.CallUser]
...
func GetUserById(userId uint64) *entity.CallUser {
	...
	v := userCacheMgr.MustGetRow(gctx.New(), userId, func(ctx context.Context) (*entity.CallUser, error) {
...
func AddUserToCache(u *entity.CallUser) {
	...
	userCacheMgr.PublishRow(gctx.New(), u.ID, u)
}
...
func GetUserFromCache(userId uint64) *entity.CallUser {
	...
	v, _ := userCacheMgr.GetRowCached(gctx.New(), userId)
	return v
}
