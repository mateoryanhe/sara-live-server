...
// SumGameBetByUser 统计指定时间范围内各用户游戏下注总额(按金额降序,最多500条).
func SumGameBetByUser(startTime, endTime time.Time) []*GameConsumeRankRow {
...
`
	...
	if err != nil {
		g.Log().Errorf(ctx, "SumGameBetByUser error: %v", err)
