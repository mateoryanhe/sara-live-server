...
// GetLiveRoomCfgForCMS 直播间配置(缓存优先,否则直查DB,不新建)
func GetLiveRoomCfgForCMS(roomId uint64) *entity.LiveRoomCfg {
	if cfg := GetLiveRoomCfgFromCache(roomId); cfg != nil {
		return cfg
	}
	return GetLiveRoomCfgFromDB(roomId)
}
