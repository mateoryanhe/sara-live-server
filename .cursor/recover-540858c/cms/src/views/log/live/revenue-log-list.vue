      ...
      <el-table v-loading="loading || exporting" :data="tableData" :element-loading-text="exportStatusTip || undefined" style="width: 100%">
        ...
        <el-table-column :label="t('pages.revenueLogList.liveRoomId')" min-width="180" prop="roomId">
          <template #default="{ row }">
            <el-button v-if="row.roomId && row.receiverId" link type="primary" @click="openAnchorDetail(row.receiverId)">
              {{ row.roomId }}
            </el-button>
            <span v-else>{{ row.roomId || '-' }}</span>
          </template>
        </el-table-column>
        <el-table-column :label="t('pages.revenueLogList.anchorAvatar')" width="80">
          <template #default="{ row }">
            <el-image
        ...
        <el-table-column :label="t('pages.revenueLogList.anchorNickname')" min-width="120" prop="receiverNickname">
        ...
        <el-table-column :label="t('pages.revenueLogList.payerUserId')" min-width="180" prop="senderId">
          <template #default="{ row }">
            <el-button v-if="canViewUserDetail && row.senderId" link type="primary" @click="openUserDetail(row.senderId)">
              {{ row.senderId }}
            </el-button>
            <span v-else>{{ row.senderId || '-' }}</span>
          </template>
        </el-table-column>
        <el-table-column :label="t('pages.revenueLogList.payerUserAvatar')" width="80">
          <template #default="{ row }">
            <el-image
...
const {canViewUserDetail, openUserDetail} = useUserDetailNav('LiveRevenueLogList')
